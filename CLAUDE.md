...
Conventions (the `standard` set in `CONVENTION_SETS`, selectable via `--conventions`): oldest card is leftmost; a colour hint marks the *newest touched* card for play (tracked via `lastHints` markers, which the server consumes when any card of that hint leaves the hand) — **every** colour hint still marked on the hand stays a pending play target (played oldest-hint-first), and a colour hint that fully identifies another touched card is read as a *reveal* (that card is played when provably playable; the newest touched card carries no play promise); a number hint means *keep* unless a card becomes provably playable; discard priority is provably-useless card → chop (oldest untouched) → forced. Own-card deduction combines hint constraints with *copy-counting*: an identity whose every copy is visible (played, discarded, or in a hand the deducer can see) is ruled out — this feeds known-playable/known-useless and the hint-givers' simulation of the receiver (which never counts the giver's own hidden hand, so it stays sound). It also saves the next player's critical chop (last remaining copy) with a number hint, and stalls with a harmless number hint at full tokens.
